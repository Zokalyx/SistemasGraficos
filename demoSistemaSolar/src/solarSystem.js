...
export class SolarSystem {
	...
	update(time) {
		...
		let sunGroup = this.sunGroup;
		let earth = this.models['earth'];
		let earthGroup = this.earthGroup;
		let moon = this.models['moon'];
		let iss = this.models['iss'];
		let apollo = this.models['apollo'];

		const RADIO_ORBITA_TIERRA = 300;
		const RADIO_ORBITA_LUNA = 100;
		const RADIO_LUNA = 10;
		const DIA = 1;
		const PERIODO_LUNAR = 28 * DIA;
		const AÑO = 365 * DIA;
		const PERIODO_ISS = 0.5 * DIA;
		const RADIO_ISS = 50;
