...
export class SceneManager {
	...
	construirVehiculo() {
		...
		for (let i = 0; i < 3; i++) {
			const tuerca = this.tuerca.clone();
			tuerca.position.set(5 * Math.cos(2 * Math.PI * i / 3), 5 * Math.sin(2 * Math.PI * i / 3), 3);
			Rueda.add(tuerca);
		}

		const ruedaIzquierda = Rueda.clone();
		ruedaIzquierda.position.set(0, 0, -27);
		ruedaIzquierda.rotation.y = Math.PI;
		this.eje.add(ruedaIzquierda);
		const ruedaDerecha = Rueda.clone();
		ruedaDerecha.position.set(0, 0, 27);
		this.eje.add(ruedaDerecha);

		this.ejeFrontal = this.eje.clone();
		this.vehiculo.add(this.ejeFrontal);
		this.ejeFrontal.position.set(20, 5, 0);

		this.ejeTrasero = this.eje.clone();
		this.vehiculo.add(this.ejeTrasero);
		this.ejeTrasero.position.set(-20, 5, 0);
